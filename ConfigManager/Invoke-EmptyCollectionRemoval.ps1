Get-CMCollection -CollectionType Device | Where-Object { ($_.MemberCount -lt 10) -and ($_.IsReferenceCollection -eq $false) -and ($_.IsBuiltIn -eq $false) -and ($_.LastChangeTime -lt (Get-Date).AddMonths(-10)) } | ForEach-Object -Begin { $RetiredCollections = New-Object -TypeName System.Collections.ArrayList } -Process {
    ...
    if ([string]::IsNullOrEmpty((Get-CMDeployment | Where-Object { $_.CollectionID -eq $TempCollection.CollectionID }))) {
        ...
        try {
            Export-CMCollection -CollectionId $_.CollectionID -ExportFilePath "\\<sccm-server>\e$\Backup\Collections\Device Collections\AutoExport\$($_.Name).mof" -ExportComment "$(Get-Date -Format d): Collection Cleanup ($($_.CollectionType)); " -Force -ErrorAction SilentlyContinue
        }
...
} -End { $RetiredCollections | Export-Csv -Path "filesystem::\\<sccm-server>\e$\Backup\Collections\Device Collections\AutoExport\AE-$(New-Guid).csv" -NoTypeInformation -Encoding UTF8 -Force }