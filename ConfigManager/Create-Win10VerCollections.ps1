...
process {
    ...
    foreach ($W10Version in $W10Versions) {
        if ($null -eq (Get-CMCollection -Name "All Windows 10 - $($W10Version.Version)" -ErrorAction SilentlyContinue)) {
            Write-Host "Creating CM collection All Windows 10 - $($W10Version.Version)"

            $Query = "select SMS_R_SYSTEM.ResourceID,SMS_R_SYSTEM.ResourceType,SMS_R_SYSTEM.Name,SMS_R_SYSTEM.SMSUniqueIdentifier,SMS_R_SYSTEM.ResourceDomainORWorkgroup,SMS_R_SYSTEM.Client from SMS_R_System inner join SMS_G_System_OPERATING_SYSTEM on SMS_G_System_OPERATING_SYSTEM.ResourceId = SMS_R_System.ResourceId where SMS_G_System_OPERATING_SYSTEM.Name like 'Microsoft Windows 10 Enterprise%' and SMS_G_System_OPERATING_SYSTEM.Version = '10.0.$(([System.version]$W10Version.OSBuild).Major)'"
