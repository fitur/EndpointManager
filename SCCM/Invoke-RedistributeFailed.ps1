...
foreach ($Package in (Get-WmiObject -Namespace root/sms/site_$($SiteCode.SiteCode) -Class SMS_PackageStatusDistPointsSummarizer -ComputerName $SiteCode.SiteServer | Where-Object {$_.State -eq 2})) {
