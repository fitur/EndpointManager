...
process {
    # Supply the ConfigMgr WebService secret key via an environment variable.
    ...
    Get-ADGroupMember -Identity $ADGroup | ForEach-Object {
        ...
        #$CMComputers = Get-CMUserDeviceAffinity -UserName "$env:USERDOMAIN\$($_.Name)" | Where-Object {(($_.Sources -eq 4) -or ($_.Sources -eq 7)) -and ($_.Types -eq 1) -and ($_.UniqueUserName -match $_.Name)}
        $CMComputers = Get-CMUserDeviceAffinity -UserName "$env:USERDOMAIN\$($_.Name)"
