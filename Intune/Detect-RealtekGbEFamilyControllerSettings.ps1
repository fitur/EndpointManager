...
.SYNOPSIS
    PowerShell script to detect the settings for Realtek USB GbE Family Controller driver.
    Realtek USB GbE Family Controller enables Green Ethernet as default, which causes intermittent disconnects.

.EXAMPLE
    .\Detect-RealtekGbEFamilyControllerSettings.ps1
...
try {
    ...
    foreach ($DesiredSetting in $AdapterSettings) {
        ...
        if (($CurrentValue -ne $AdapterValues[$AdapterSettings.IndexOf($DesiredSetting)]) -and (![string]::IsNullOrEmpty($CurrentValue))) {
            Write-Verbose "$DesiredSetting is incorrect." -Verbose
            $Remediation += 1
        }
        else {
            Write-Verbose "$DesiredSetting is correct or N/A." -Verbose
        }
    }
    if ($Remediation -gt 0) {
